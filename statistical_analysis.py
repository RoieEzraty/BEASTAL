...
def final_err(loss_scalar_in_t: NDArray[np.float_], samples: int = 40) -> float:
    """
    ...
    """
    return float(np.mean(loss_scalar_in_t[-samples:]))


def calculate_accuracy_1sample(output: NDArray[np.float_], targets_mat: NDArray[np.float_],
                               target_i: NDArray[np.float_]) -> float:
    """
    ...
    predicted_class = int(np.argmin(l2_vec))
    target_class = int(np.argmax(target_i))
    return float(predicted_class == target_class)


def power_dissip(u: NDArray[np.float_], R: NDArray[np.float_]) -> float:
    """
...
def power_dissip_norm(u: NDArray[np.float_], R: NDArray[np.float_], input: NDArray[np.float_]) -> float:
    """
...
def mov_ave(data: ArrayLike, window_size: int) -> NDArray[np.float_]:
    """
